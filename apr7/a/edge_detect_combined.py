...
def detect_edges(input_path, output_path):
    ...
    # combine X and Y into a single magnitude: sqrt(x^2 + y^2) - typical of sobel algorithm
    magnitude = np.sqrt(edges_x ** 2 + edges_y ** 2)

    # actually makes the image in the dir - important: normalize_to_image is a cv2 function which:
    # takes all the inputs of the array, makes the smallest map to 0, the largest map to 255, and the rest are fit linearly.
    cv2.imwrite(str(output_path), normalize_to_image(magnitude))


for image_path in INPUT_DIR.glob("*.jpg"):
    output = OUTPUT_DIR / f"{image_path.stem}_edges.jpg"
    detect_edges(image_path, output)
